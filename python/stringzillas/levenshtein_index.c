...
static char const doc_LevenshteinIndexUTF8[] =
    ...
    "Build an immutable exact Unicode-codepoint dictionary index from validated UTF-8. Calling the object with "
    "``(query, bound=None)`` returns an unordered list of ``(dictionary_id, distance)`` pairs.";
