...
static char const doc_LevenshteinIndex[] =
    ...
    "Build an exact byte-level dictionary index for repeated searches. Calling the object with "
    "``(query, bound=None)`` returns an unordered list of ``(dictionary_id, distance)`` pairs. Duplicate "
    "dictionary values retain distinct IDs.";
