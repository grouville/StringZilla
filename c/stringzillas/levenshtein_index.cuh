...
using levenshtein_index_utf8_t = szs::levenshtein_index_utf8<>;
...
struct levenshtein_index_utf8_search_state_t {
    levenshtein_index_utf8_t::scratch_t scratch;
    levenshtein_index_utf8_t::matches_t matches;
};
...
SZ_API_RUNTIME sz_status_t szs_levenshtein_index_utf8_init(                     //
    sz_sequence_t const *dictionary, sz_size_t max_distance,                    //
    sz_size_t deletion_max_word_length, sz_memory_allocator_t const *alloc,     //
    szs_levenshtein_index_utf8_t *index_punned, char const **error_message) {
    sz_assert_(dictionary != nullptr && "Dictionary must not be null");
    return szs_levenshtein_index_init_<levenshtein_index_utf8_t>(sz_sequence_as_cpp_container_t {dictionary},
                                                                 max_distance, deletion_max_word_length, alloc,
                                                                 index_punned, error_message);
}
...
SZ_API_RUNTIME sz_status_t szs_levenshtein_index_utf8_init_u32tape(              //
    ...
    szs_levenshtein_index_utf8_t *index_punned, char const **error_message) {
    sz_assert_(dictionary != nullptr && "Dictionary must not be null");
    return szs_levenshtein_index_init_<levenshtein_index_utf8_t>(
        sz_sequence_u32tape_as_cpp_container_t {dictionary}, max_distance, deletion_max_word_length, alloc,
        index_punned, error_message);
}
...
SZ_API_RUNTIME sz_status_t szs_levenshtein_index_utf8_init_u64tape(              //
    ...
    sz_assert_(dictionary != nullptr && "Dictionary must not be null");
    return szs_levenshtein_index_init_<levenshtein_index_utf8_t>(
        sz_sequence_u64tape_as_cpp_container_t {dictionary}, max_distance, deletion_max_word_length, alloc,
        index_punned, error_message);
}

SZ_API_RUNTIME sz_status_t szs_levenshtein_index_utf8_search_init( //
    szs_levenshtein_index_utf8_t index_punned,                    //
    szs_levenshtein_index_utf8_search_t *search_punned, char const **error_message) {
    sz_assert_(index_punned != nullptr && "Index must be initialized");
    sz_assert_(search_punned != nullptr && *search_punned == nullptr && "Search state must be uninitialized");
    auto *search = new (std::nothrow) levenshtein_index_utf8_search_state_t;
    if (!search)
        return propagate_error(sz::status_t::bad_alloc_k, error_message,
                               "Failed to allocate UTF-8 Levenshtein index search state");
    *search_punned = reinterpret_cast<szs_levenshtein_index_utf8_search_t>(search);
    return propagate_error(sz::status_t::success_k, error_message);
}

SZ_API_RUNTIME sz_status_t szs_levenshtein_index_utf8_find(                                  //
    szs_levenshtein_index_utf8_t index_punned, szs_levenshtein_index_utf8_search_t search_punned, //
    sz_cptr_t query, sz_size_t query_length, sz_size_t bound,                                 //
    szs_levenshtein_index_match_t const **matches, sz_size_t *matches_count,                  //
    char const **error_message) {
    sz_assert_(index_punned != nullptr && "Index must be initialized");
    ...
    *matches_count = 0;
    ...
    auto const *index = reinterpret_cast<levenshtein_index_utf8_t const *>(index_punned);
    auto *search = reinterpret_cast<levenshtein_index_utf8_search_state_t *>(search_punned);
    sz::status_t const status = index->find({query, query_length}, static_cast<sz::u8_t>(bound), search->scratch,
                                            search->matches);
    if (status != sz::status_t::success_k)
        return propagate_error(status, error_message, "Failed to search UTF-8 Levenshtein dictionary index");
    *matches = search->matches.data();
    *matches_count = search->matches.size();
    return propagate_error(sz::status_t::success_k, error_message);
}

SZ_API_RUNTIME void szs_levenshtein_index_utf8_search_free(szs_levenshtein_index_utf8_search_t search_punned) {
    sz_assert_(search_punned != nullptr && "Search state must be initialized");
    delete reinterpret_cast<levenshtein_index_utf8_search_state_t *>(search_punned);
}

SZ_API_RUNTIME void szs_levenshtein_index_utf8_free(szs_levenshtein_index_utf8_t index_punned) {
    sz_assert_(index_punned != nullptr && "Index must be initialized");
    delete reinterpret_cast<levenshtein_index_utf8_t *>(index_punned);
}
