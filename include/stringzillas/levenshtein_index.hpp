...
class basic_levenshtein_index {
  ...
  private:
    ...
    struct dense_equality_entry_t {
        symbol_t symbol;
        u64_t mask;
    };
    struct dense_query_t {
        u64_t equality_dense[256] = {};
        dense_equality_entry_t equality_sparse[64];
        size_t equality_sparse_count = 0;
        size_t words_count = 0;
    };

    template <typename scratch_type_>
    static status_t prepare_dense_query_(span<symbol_t const> query, scratch_type_ &scratch,
                                         dense_query_t &state) noexcept {
        state.words_count = query.size() / 64 + (query.size() % 64 != 0);
        if (query.size() <= 64) {
            if constexpr (sizeof(symbol_t) == 1)
                for (size_t index = 0; index != query.size(); ++index)
                    state.equality_dense[static_cast<u8_t>(query[index])] |= u64_t(1) << index;
            else {
                for (size_t index = 0; index != query.size(); ++index)
                    state.equality_sparse[index] = dense_equality_entry_t {query[index], u64_t(1) << index};
                std::sort(state.equality_sparse, state.equality_sparse + query.size(),
                          [](auto const &first, auto const &second) { return first.symbol < second.symbol; });
                for (size_t index = 0; index != query.size(); ++index) {
                    if (state.equality_sparse_count &&
                        state.equality_sparse[state.equality_sparse_count - 1].symbol ==
                            state.equality_sparse[index].symbol)
                        state.equality_sparse[state.equality_sparse_count - 1].mask |=
                            state.equality_sparse[index].mask;
                    else
                        state.equality_sparse[state.equality_sparse_count++] = state.equality_sparse[index];
                }
            }
            return status_t::success_k;
        }
        ...
        if constexpr (sizeof(symbol_t) == 1) {
            if (state.words_count > std::numeric_limits<size_t>::max() / 256)
                return status_t::overflow_risk_k;
            if (status_t status = scratch.dense_match_masks.try_resize(state.words_count * 256);
                status != status_t::success_k)
                return status;
            std::fill(scratch.dense_match_masks.begin(), scratch.dense_match_masks.end(), u64_t(0));
            for (size_t index = 0; index != query.size(); ++index)
                scratch.dense_match_masks[static_cast<u8_t>(query[index]) * state.words_count + index / 64] |=
                    u64_t(1) << (index % 64);
        }
        else {
            ...
            if (status_t status = scratch.dense_symbols.try_resize(
                ...
                status != status_t::success_k)
                return status;
            if (scratch.dense_symbols.size() > std::numeric_limits<size_t>::max() / state.words_count)
                return status_t::overflow_risk_k;
            if (status_t status =
                    scratch.dense_match_masks.try_resize(scratch.dense_symbols.size() * state.words_count);
                status != status_t::success_k)
                return status;
            std::fill(scratch.dense_match_masks.begin(), scratch.dense_match_masks.end(), u64_t(0));
            for (size_t index = 0; index != query.size(); ++index) {
                symbol_t const *symbol =
                    std::lower_bound(scratch.dense_symbols.begin(), scratch.dense_symbols.end(), query[index]);
                size_t const symbol_index = static_cast<size_t>(symbol - scratch.dense_symbols.begin());
                scratch.dense_match_masks[symbol_index * state.words_count + index / 64] |=
                    u64_t(1) << (index % 64);
            }
        }
        if (status_t status = scratch.dense_positive.try_resize(state.words_count); status != status_t::success_k)
            return status;
        return scratch.dense_negative.try_resize(state.words_count);
    }

    template <typename scratch_type_>
    static void distance_dense_(span<symbol_t const> query, span<symbol_t const> candidate, size_t cutoff,
                                dense_query_t const &state, scratch_type_ &scratch, size_t &distance,
                                bool &rejected) noexcept {
        rejected = false;
        if (query.empty()) {
            distance = candidate.size();
            return;
        }
        distance = query.size();
        if (query.size() <= 64) {
            ...
            for (size_t index = 0; index != candidate.size(); ++index) {
                ...
                if constexpr (sizeof(symbol_t) == 1)
                    matches_mask = state.equality_dense[static_cast<u8_t>(candidate[index])];
                else {
                    dense_equality_entry_t const *entry = std::lower_bound(
                        state.equality_sparse, state.equality_sparse + state.equality_sparse_count, candidate[index],
                        [](dense_equality_entry_t const &entry, symbol_t symbol) { return entry.symbol < symbol; });
                    if (entry != state.equality_sparse + state.equality_sparse_count &&
                ...
                u64_t const horizontal_positive = negative | ~(differences | positive);
                u64_t const horizontal_negative = positive & differences;
                distance += (horizontal_positive & top) != 0;
                distance -= (horizontal_negative & top) != 0;
                u64_t const shifted_positive = (horizontal_positive << 1) | 1;
                u64_t const shifted_negative = horizontal_negative << 1;
                positive = shifted_negative | ~(differences | shifted_positive);
                negative = shifted_positive & differences;
                if (cutoff != std::numeric_limits<size_t>::max() &&
                    distance > cutoff + candidate.size() - index - 1) {
                    rejected = true;
                    return;
                }
            }
        ...
        for (size_t index = 0; index != candidate.size(); ++index) {
            ...
            if constexpr (sizeof(symbol_t) == 1)
                matches_row = scratch.dense_match_masks.data() +
                              static_cast<u8_t>(candidate[index]) * state.words_count;
            else {
                symbol_t const *symbol =
                    std::lower_bound(scratch.dense_symbols.begin(), scratch.dense_symbols.end(), candidate[index]);
                if (symbol != scratch.dense_symbols.end() && *symbol == candidate[index])
                    matches_row = scratch.dense_match_masks.data() +
                                  static_cast<size_t>(symbol - scratch.dense_symbols.begin()) * state.words_count;
            }
            u64_t positive_carry = 1, negative_carry = 0;
            for (size_t word = 0; word != state.words_count; ++word) {
                ...
                u64_t const diagonal_zero = (((matched_with_carry & scratch.dense_positive[word]) +
    ...
    status_t find_dense_(span<symbol_t const> query, u8_t bound, scratch_type_ &scratch,
        ...
        for (u32_t id = 0; id != size(); ++id) {
            ...
            if (length_difference > bound) continue;
            size_t distance = 0;
            bool rejected = false;
            distance_dense_(query, candidate, bound, state, scratch, distance, rejected);
            if (!rejected && distance <= bound)
