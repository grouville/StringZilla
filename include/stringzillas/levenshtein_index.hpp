 ...
 *
 *  Adaptively builds a lossless deletion-neighborhood filter or a radix trie for strings over fixed-width symbols.
 *  Hash collisions can only add verifier work; they cannot alter returned matches. Query-local generations,
 *  automaton transitions, DP rows, and output are explicit, making one immutable index safe to search concurrently.
 */
...
/**
 *  @brief Owns an immutable dictionary and finds every entry within an inclusive Levenshtein bound.
 *
 *  Building can be expensive, but the resulting index can be shared by any number of readers. Each reader must use
 *  its own @c scratch_t and output vector. Matches contain the original dictionary ID and exact distance. Their order
 *  is unspecified, and duplicate dictionary entries keep their distinct IDs.
 */
...
class basic_levenshtein_index {
  ...
  public:
    /** @brief Reusable query memory owned by one reader. */
    class scratch_t {
    ...
    /**
     *  @brief Replaces the dictionary after building a complete new index.
     *  @param[in] dictionary Random-access strings exposing @c data() and @c size().
     *  @param[in] max_distance Largest bound accepted by @c find().
     *  @param[in] deletion_max_word_length Optional cutoff for the short-string index. The default chooses between
     *  that index and the trie from the expected number of deletion records.
     *  @return Success or an error. On error, the previous index remains unchanged.
     */
     ...
     *  @note Clears @p matches before searching. The bound must not exceed the value used to build the index.
     */
