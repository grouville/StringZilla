...
int main(void) {
    ...
    matches = (szs_levenshtein_index_match_t const *)1;
    matches_count = 1;
    if (szs_levenshtein_index_find(index, search, "book", 4, SZ_SIZE_MAX, &matches, &matches_count,
                                   &error_message) != sz_unexpected_dimensions_k)
        return 7;
    if (matches != NULL || matches_count != 0) return 8;
