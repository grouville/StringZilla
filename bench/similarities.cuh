...
using ashvardanian::stringzillas::levenshtein_within_icelake_t;
...
void bench_levenshtein_within(environment_t const &env) {
    ...
    for (shape_t const &shape : shapes) {
        ...
        for (std::size_t bound : bounds)
            bench_unary(
                env, "levenshtein_within_icelake_k"s + std::to_string(bound) + ":"s + shape_label,
                call_distances_baseline, // full distances, thresholded at `bound` by the equality check
                similarities_callable_for<similarities_within_t, levenshtein_within_icelake_t, forkunion_executor_t &>(
                    env, results_within_accelerated, shape, levenshtein_within_icelake_t {bound}, pool),
                callable_no_op_t {},                    // preprocessing
                similarities_within_equality_t {bound}) // equality check
                .log(distances_baseline);
#endif
